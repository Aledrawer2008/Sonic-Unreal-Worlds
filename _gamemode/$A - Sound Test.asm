; ---------------------------------------------------------------------------
; Sound Test Screen
; credits to vladikcomper for the basis from EraZoR
; ---------------------------------------------------------------------------
...
SoundTestScreen:
	...
	; Clear object RAM
		lea	(v_objspace), a1
		moveq	#0, d0
		move.w	#(v_objspace_end-v_objspace)/$40-1, d1
	...
	endr
		...
		lea	(v_pal_dry), a0		; fill current palette with black
		moveq	#0, d0
	rept 128/4
		move.l	d0, (a0)+
	endr
	...
	; Load/render Plane B
		...
		jsr	EniDec
		writeVRAM $FF0000, $2000, SoundTest_PlaneB_VRAM
	...
	; Load piano mappings
		...
		locVRAM	SoundTest_PlaneA_VRAM+$A00, d0
		lea	($FF0000).w, a1
	...
	; Load header mappings
		...
		locVRAM	SoundTest_PlaneA_VRAM+$80+10*2, d0
		lea	$FF0000, a1
		moveq	#20-1, d1
		moveq	#2-1, d2
		jsr	TilemapToVRAM
	...
	; Draw top separator
		...
		.fill_value:	equr	d0
		.(vdp_control_port):	equr	a5
		.vdp_data:	equr	a6

		lea	(vdp_control_port), .(vdp_control_port)
		lea	VDP_Data-(vdp_control_port)(.(vdp_control_port)), .vdp_data
		vram	SoundTest_PlaneA_VRAM+$180, (.(vdp_control_port))
...
SoundTest_MainLoop:
		st	(v_vbla_routine)
		jsr	WaitForVBla
		bsr.w	SoundTest_VDeform_Update
		SoundTest_InitWriteRequests
		jsr	ExecuteObjects
		jsr	BuildSprites
		SoundTest_FinalizeWriteRequests a0
		bsr.w	SoundTest_Visualizer_Update
		bsr.w	SoundTest_UpdateFading

		tst.b	SoundTest_ExitFlag			; exit flag set?
		beq.w	SoundTest_MainLoop			; if not, branch
		tst.b	SoundTest_FadeCounter			; faded out?
		bne.w	SoundTest_MainLoop			; if not, branch
	; fallthrough

; ---------------------------------------------------------------------------
SoundTest_Exit:
		move.w	#$8C81, (vdp_control_port)		; disable S&H
...
SoundTest_SetupVDP:
	...
	.setup_vdp_loop:
		move.w	(.vdp_regs)+, (.(vdp_control_port))
		dbf	.loop_cnt, .setup_vdp_loop

	; Clear-fill Plane A (the logical one)
	.fill_value:	equr	d0

	vram	SoundTest_PlaneA_VRAM, (.(vdp_control_port))
	...
	move.l	#(.hscroll_value<<16)|(.hscroll_value), d0
	move.w	#$8F20, (.(vdp_control_port))

	vram	$FC00, (.(vdp_control_port))
	moveq	#28/2-1, d1
	.fill_plane_a_hscroll:
		move.l	d0, (.vdp_data)
		dbf	d1, .fill_plane_a_hscroll

	vram	$FC02, (.(vdp_control_port))
	moveq	#28/2-1, d1
...
SoundTest_Visualizer_Init:
	...
	lea	(vdp_control_port)-VDP_Data(.vdp_data), .(vdp_control_port)
	...
	move.l	#(($9400+((.dma_len-1)>>8))<<16)|($9300+((.dma_len-1)&$FF)), (.(vdp_control_port))
	move.l	#(($9700+$80)<<16)|$8F01, (.(vdp_control_port))
	move.l	#($40000000+(((.dma_dest)&$3FFF)<<16)+(((.dma_dest)&$C000)>>14))|$80, (.(vdp_control_port))
	move.w	#$0000, (.vdp_data)

	; Decompress plane mappings
	move.l	.(vdp_control_port), -(sp)
	lea	SoundTest_PianoSheet_MapKosp(pc), a0
	lea	$FF0000, a1
	jsr	KosPlusDec
	move.l	(sp)+, .(vdp_control_port)

	assert.l a1, eq, #$FF0000+$1000

	; Wait for generate tiles DMA to finish (it was running in parallel!)
	.wait_dma:
		move.w	(.(vdp_control_port)), ccr
		bvs.s	.wait_dma
...
SoundTest_Visualizer_TransferPixelBufferToVRAM:
	...
	move.w	#$8F80, (.(vdp_control_port))		; DMA advances every other tile ($80 bytes)

	move.l	.dma_size_regs, (.(vdp_control_port))	; VDP => DMA size regs
	movep.w	.dma_src, 1(sp)
	move.w	#$977F, (.(vdp_control_port))		; VDP => DMA's source high byte
	move.l	(sp), (.(vdp_control_port))		; VDP => DMA's source mid and low byte
	move.w	.write_dest, (.(vdp_control_port))	; VDP => DMA destination command (high word)
	move.w	4(sp), (.(vdp_control_port))		; VPD => DMA destination command (low word)
