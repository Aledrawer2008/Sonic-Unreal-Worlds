...
; ----------------------------------------------------------------------------
; OST Variables:
Obj2C_move_timer	= $2A	; time to wait before turning around
Obj2C_bubble_timer	= $2C	; time to wait before producing a bubble
; Sprite_36DAC:
Obj2C:
		moveq	#0,d0
...
; off_36DBA:
Obj2C_Index:
		dc.w Obj2C_Init-Obj2C_Index		; 0 - Initialize object variables
		dc.w Obj2C_Main-Obj2C_Index		; 2 - Moving back and forth until Sonic or Tails approach
		dc.w Obj2C_Waiting-Obj2C_Index	; 4 - Stopped, opening and closing mouth
		dc.w Obj2C_Charge-Obj2C_Index	; 6 - Charging at Sonic or Tails
; ===========================================================================
; loc_36DC2:
Obj2C_Init:
		...
		move.w	#$200,Obj2C_move_timer(a0)
		move.w	#$50,Obj2C_bubble_timer(a0)
...
; loc_36DE4:
Obj2C_Main:
		subq.b	#1,Obj2C_bubble_timer(a0)
		bne.s	.skip1			; branch, if timer isn't done counting down
		bsr.w	Obj2C_MakeBubble
.skip1:
		subq.w	#1,Obj2C_move_timer(a0)
		bpl.s	.skip2			; branch, if timer isn't done counting down
		move.w	#$200,Obj2C_move_timer(a0)	; else, reset timer...
...
.skip2:
		...
		move.w	d2,d4
		move.w	d3,d5
		bsr.w	Obj2C_TestCharacterPos	; are Sonic or Tails close enough to attack?
		bne.s	Obj2C_PrepareCharge	; if yes, prepare to charge at them
		jmp	RememberState
; ===========================================================================
; loc_36E20
Obj2C_PrepareCharge:
		addq.b	#2,obRoutine(a0)	; => Obj2C_Waiting
		move.b	#$10,Obj2C_move_timer(a0)	; time to wait before charging at the player
		clr.w	obVelX(a0)		; stop movement
		jmp	RememberState
; ===========================================================================
; loc_36E32:
Obj2C_Waiting:
		subq.b	#1,Obj2C_move_timer(a0)
		bmi.s	Obj2C_MoveTowardsPlayer		; branch, if wait time is over
		bra.w	Obj2C_Animate
; ===========================================================================
; loc_36E3C:
Obj2C_MoveTowardsPlayer:
		addq.b	#2,obRoutine(a0)	; => Obj2C_Charge
		bsr.w	Obj_GetOrientationToPlayer
		lsr.w	#1,d0		; set speed based on closest character
		move.b	Obj2C_HorizontalSpeeds(pc,d0.w),obVelX(a0)	; horizontal
		addi.w	#$10,d3
		...
		move.b	Obj2C_VerticalSpeeds(pc,d1.w),1+y_vel(a0)	; vertical
.skip:
		bra.w	Obj2C_Animate
; ===========================================================================
; byte_36E62:
Obj2C_HorizontalSpeeds:
		dc.b  -2	; 0 - player is left from object -> move left
		dc.b   2	; 1 - player is right from object -> move right
; byte_36E64:
Obj2C_VerticalSpeeds:
...
; loc_36E78:
Obj2C_MakeBubble:
		move.w	#$50,Obj2C_bubble_timer(a0)	; reset timer
		jsr	FindFreeObj
		bne.s	.return
...
; ===========================================================================
; loc_36EB2:
Obj2C_TestCharacterPos:
		addi.w	#$20,d3
		cmpi.w	#$40,d3			; is character too low?
		bhs.s	Obj2C_DoNotCharge	; if yes, branch
		tst.w	d2			; is character to the left?
		bmi.s	Obj2C_TestPosLeft	; if yes, branch
		tst.w	obVelX(a0)		; is object moving left, towards character?
		bpl.s	Obj2C_DoNotCharge	; if not, branch
		bra.w	Obj2C_TestHorizontalDist
; ===========================================================================
; loc_36ECA:
Obj2C_TestPosLeft:
		tst.w	obVelX(a0)		; is object moving right, towards character?
		bmi.s	Obj2C_DoNotCharge	; if not, branch
		neg.w	d2			; get absolute value

; loc_36ED2:
Obj2C_TestHorizontalDist:
		cmpi.w	#$20,d2			; is distance less than $20?
		blo.s	Obj2C_DoNotCharge	; if yes, don't attack
		cmpi.w	#$A0,d2			; is distance less than $A0?
		blo.s	Obj2C_PlayerInRange	; if yes, attack
