...
Pow_ChkShoes:
		...
		bne.s	Pow_ChkShield
		tst.b	(f_supersonic).w ; Prevent Sonic from getting invincibility if Super
        bne.w 	TenRings
...
Pow_ChkInvinc:
		...
		tst.b	(f_supersonic).w ; Prevent Sonic from getting invincibility if Super
        bne.w 	TenRings
		add.w	#$4B0,(v_player+$32).w ; time limit for the power-up
		tst.b	(v_invinc).w
		bne.s	Pow_NoMusic
...
TenRings:
		move.b	#10,d1
AddRings:
		addi.w	d1,(v_rings).w	; add 10 rings to the number of rings you have
...
Pow_ChkKey:		; Gives you the Special stage's Key
		cmpi.b	#7,d0		; does monitor contain the SS Key?
		bne.s	Pow_ChkFiveRings
...
.rings:
		...
		add.w	#20,(v_rings).w	; add 1 to rings
		ori.b	#1,(f_ringcount).w ; update the rings counter
        move.w 	#sfx_GiantRing,d0
        jmp    	(PlaySound_Special).l    ; play Giant ring sound

Pow_ChkFiveRings: 		; Gives 5 rings
    	cmpi.b	#8,d0        ; does monitor contain 5 rings?
    	bne.s	Pow_ChkEnd
		move.b	#5,d1
		bra.s	AddRings
