...
VBla_Exit:
		...
		tst.w	(Timer_Boredom).w			; Is the bored timer active?
		beq.s	.end			; If not, branch
		addq.w	#1,(Timer_Boredom).w			; Increment bored timer
.end:
		movem.l	(sp)+,d0-a6
		rte	
; ===========================================================================
...
; ---------------------------------------------------------------
; Subroutine to load the art via DPLCs
; ---------------------------------------------------------------
...
Generic_LoadGfx:
        	...
        	dbf 	d5,Generic_LoadGfx  ; repeat for number of entries
...
; ---------------------------------------------------------------------------
; Unused underwater palette loading subroutines
; ---------------------------------------------------------------------------
...
; Palette data
; ---------------------------------------------------------------------------
Pal_SegaBG:	binclude	"palette/Sega.bin"
Pal_Title:	binclude	"palette/Title Screen.bin"
Pal_TEncore:	binclude	"palette/Team Encore.bin"
Pal_LevelSel:	binclude	"palette/Level Select.bin"
Pal_Sonic:	binclude	"palette/Sonic.bin"
Pal_Amy:	binclude	"palette/Amy.bin"
Pal_SonAmy:	binclude	"palette/Sonic & Amy.bin"
...
Pal_SpecialAmy:	binclude	"palette/Special Stage Alt.bin"
Pal_SSResult:	binclude	"palette/Special Stage Results.bin"
Pal_Continue:	binclude	"palette/Special Stage Continue Bonus.bin"
Pal_ContinueAlt: binclude	"palette/Special Stage Continue Bonus Alt.bin"
...
MusicList:
        ...
        dc.b bgm_EIZ2   ; GHZ2
        dc.b bgm_EIZ3   ; GHZ3
        dc.b bgm_Tutorial ; GHZ4 (Tutorial)
        dc.b bgm_SPZ1   ; LZ1
        dc.b bgm_SPZ2   ; LZ2
        dc.b bgm_SPZ3   ; LZ3
        dc.b bgm_HLZ3   ; LZ4 (Mecha Fortress Act 3)
        dc.b bgm_BTZ1   ; MZ1
        dc.b bgm_BTZ2   ; MZ2
        dc.b bgm_BTZ3   ; MZ3
        dc.b bgm_BTZ3   ; MZ4 (Unused)
        dc.b bgm_SPA1   ; SLZ1
        dc.b bgm_SPA2   ; SLZ2
        dc.b bgm_SPA3   ; SLZ3
        dc.b bgm_TAZ	; SLZ4 (Train Assault)
        dc.b bgm_BCZ1   ; SYZ1
        dc.b bgm_BCZ2   ; SYZ2
        dc.b bgm_BCZ3   ; SYZ3
        dc.b bgm_BCZ3   ; SYZ4 (Unused)
        dc.b bgm_HLZ1   ; SBZ1
        dc.b bgm_HLZ2   ; SBZ2
        dc.b bgm_FBZ    ; SBZ3 (Final Battle)
        dc.b bgm_UBZ	; SBZ4 (Unreal Battle)
        dc.b bgm_EIZ1   ; (Unused)
        dc.b bgm_EIZ1   ; (Unused)
        dc.b bgm_EIZ1   ; (Unused)
        dc.b bgm_EIZ1   ; (Unused)
        even
...
SignpostArtLoad:
		...
		cmpi.b	#1,(v_act).w	; is act 3 or higher?
		bgt.w	.exit		; if yes, branch
		...
		locVRAM $96C0
		lea	(Nem_Bonus).l,a0 ; load Bonuses patterns
		jmp	NemDec
...
; ===========================================================================
		...
		include	"_gamemode/3 - Special Stage.asm"
		include	"_gamemode/4 - Continue Screen.asm"
...
Map_ContScr:	include	"_maps/Continue Screen.asm"
		...
		include	"_gamemode/5 - Ending Sequence.asm"
...
Map_EAmy:	include	"_maps/Ending Sequence Amy.asm"
Map_ECha:	include	"_maps/Ending Sequence Emeralds.asm"

		include	"_gamemode/6 - Credits.asm"
...
Map_EEgg:	include	"_maps/Try Again & End Eggman.asm"
		...
		include "_inc/LoadRingFrame.asm"
		include	"_inc/ProcessBGScroll.asm"
...
LoadLevelArt:
		...
		ror.b	#2,d0
    	lsr.w   #4,d0           ; shift 2 bits right
        move.l  LLA_ArtList(pc,d0.w),a0 ; get correct entry from art file list

        locVRAM	0,d4       ; set "VRAM Write to $0000"
        jsr   LoadCompArt     ; load comper compressed art
...
LLA_ArtList:
		...
		dc.l Kos_SBZ, Kos_SBZ, Kos_SBZ, Kos_UBZ
		dc.l Kos_Ending, Kos_Ending, Kos_Ending, Kos_Ending

; ---------------------------------------------------------------------------
; Subroutine to load the tiles of the zone
; ---------------------------------------------------------------------------

LoadZoneTiles:
		moveq	#0,d0			; Clear d0
		move.w	(v_zone).w,d0		; Load number of current zone
		ror.b   #2,d0
		lsr.w	#2,d0			; Multiply by $10, converting the zone ID into an offset
		lea	(LevelHeaders).l,a2	; Load LevelHeaders's address
		...
		lea	($FF0000).l,a1		; Load v_256x256/StartOfRAM into a1
		jsr	KosDec			; Decompress a0 to a1
		...
		andi.w	#$F,d7			; Only keep the low nibble of low byte , this nibble is how many $1000 bytes the decompressed art is
...
LevelDataLoad:
		moveq	#0,d0
		move.w	(v_zone).w,d0
        bsr.w   LoadLevelArt        ; load level tiles
		ror.b   #2,d0
		lsr.w	#2,d0
...
Nem_Platforms:	include "artnem/GHZ Plataforms.asm"
Map_Plat_Tut:	include	"_maps/Platforms (Tutorial).asm"
Map_Plat_GHZ:	include	"_maps/Platforms (GHZ, new).asm"
...
Ledge_SlopeData:
		...
		even
Nem_Ledge:	binclude	"artnem/GHZ Collapsing Ledge.bin"
		even
Map_Ledge:	include	"_maps/Collapsing Ledge (New).asm"
...
;  d2 = Sonic's horizontal distance to object
;  d3 = Sonic's vertical distance to object
;
; writes:
;  d0, d1, d2, d3, d4, d5
;  a1
...
.skip:
		move.w	obY(a0),d3
		sub.w	obY(a1),d3	; vertical distance to Sonic
		bhs.s	.return	; branch, if enemy is under
		addq.w	#2,d1
.return:
		rts
...
GRingDynPLC:	include	"_maps/Giant Ring - DPLCs.asm"
...
Map_Monitor:	include	"_maps/Monitor.asm"
		...
		include	"_incObj/02 Planet.asm"
		include	"_incObj/03 Team Encore Screen.asm"
...
Map_Menu:	include	"_maps/Title Menu.asm"
...
; ---------------------------------------------------------------------------
; Sprite mappings - Zone Results
; ---------------------------------------------------------------------------
Map_Got:
		include "_maps/Zone results.asm"
; ---------------------------------------------------------------------------
; Sprite mappings - special stage results screen
; ---------------------------------------------------------------------------
Map_SSR:	include	"_maps/SS Results.asm"

Map_SSR_Alt: include	"_maps/SS Results Amy.asm"
...
Map_SmashAlt:	include	"_maps/Smashable Walls Alt.asm"
...
BuildSpr_FlipXY:
	...
	.return:
		...
		include	"_incObj/sub ChkSizedObjectVisible.asm"
...
Map_Booster:	include	"_maps/Boosters.asm"
...
Map_Sign:	include	"_maps/Signpost_Maps.asm"
...
DPLC_SStars:	include	"_maps/Super Stars DPLC.asm"
...
Sonic_ChkBoredom:
		lea	(Timer_Boredom).w,a1	; Get boredom timer (Timer_Boredom's length is a word)

		cmpi.b	#id_Wait,obAnim(a0)		; Is the player idle?
...
.TimerGoing:
		cmpi.w	#10800,(a1)			; Have 3 minutes passed?
		bcs.s	.End				; If not, branch
	
		move.w	#0,(a1)				; Stop the timer

		move.b	#id_GiveUp,obAnim(a0)	; Set the player's animation accordingly
		move.b	#0,obPriority(a0)

		move.b	#1,(v_lives).w		; Make it so a game over happens

		move.w	#-$500,obVelY(a0)	; Make the player jump
...
; ===========================================================================
		include	"_incObj/01 Player.asm"

		include	"_incObj/Sonic Display.asm"
		include	"_incObj/Sonic RecordPosition.asm"
		include	"_incObj/Sonic Water.asm"
; ---------------------------------------------------------------------------
; Subroutine to	load the character map
; ---------------------------------------------------------------------------

; ||||||||||||||| S U B	R O U T	I N E |||||||||||||||||||||||||||||||||||||||

Character_Mappings:
		move.b	(v_character).w,d0	; set Character to d0
		lsl.w	#2,d0
		tst.b	(f_supersonic).w	; is character super?
		bne.s	.Super	; if yes, branch
		move.l	.Index1(pc,d0.w),obMap(a0) ; load the map
		rts

.Index1:
		dc.l	Map_Sonic		; Sonic
		dc.l	Map_Amy			; Amy Rose

.Super:
		move.l	.Index2(pc,d0.w),obMap(a0) ; load the map
		rts

.Index2:
		dc.l	Map_SuperSonic	; Super Sonic
		dc.l	Map_SuperAmy 	; Super Amy Rose
; End of function Map_Character
        ...
        include "_incObj/Sonic_Spindash.asm"
		include	"_incObj/Sonic Peelout.asm"
		include	"_incObj/Amy HighJump.asm"
...
Map_Train:	include	"_maps/Train Boss.asm"
...
Map_LampB:	include	"_maps/LamppostB.asm"
...
Map_BgEggman:	include "_maps/EggmanBg.asm"
Map_BossItems:	include	"_maps/Boss Items.asm"
Map_TFB:	include	"_maps/Egg Phantom.asm"
		include	"_anim/Egg Phantom.asm"
...
Eni_Title:	binclude	"tilemaps_u/Title Screen.bin" ; title screen foreground (mappings)
		even
Eni_JapTitle:	binclude	"tilemaps_u/Title Screen JP.bin" ; title screen foreground (mappings)
		even
...
Nem_JapNames:	binclude	"artnem/Hidden Japanese Credits.bin"
		even
Eni_Menu:	binclude	"tilemaps/Level Select BG.bin" ; Japanese credits (mappings)
		even
Nem_Menu:	binclude	"artnem/Level Select BG.bin"
		even
Nem_RingFire:	binclude	"artnem/Ring And Fire.bin"
		even

		align $20000
Map_Sonic:	include	"_maps/Sonic.asm"
Map_SuperSonic:	include	"_maps/Super Sonic.asm"
Map_Amy:	include	"_maps/Amy.asm"
Map_SuperAmy:	include	"_maps/Super Amy.asm"
SonicDynPLC:	include	"_maps/Sonic - Dynamic Gfx Script.asm"
SprSonicDynPLC:	include	"_maps/Super Sonic - Dynamic Gfx Script.asm"
AmyDynPLC:	include	"_maps/Amy - Dynamic Gfx Script.asm"
...
Nem_SSRedWhite:	binclude	"artnem/Special Red-White.bin" ; special stage red/white block
		even
Nem_SSZone1:	binclude	"artnem/Special ZONE1.bin" ; ZONE1 block
		even
...
Nem_ResultEm:	binclude	"artnem/Special Result Emeralds.bin" ; chaos emeralds on special stage results screen
		even
; ---------------------------------------------------------------------------
; Nemesis-Compressed graphics - GHZ stuff
; ---------------------------------------------------------------------------
...
Nem_MzGlass:	binclude	"artnem/MZ Green Glass Block.bin"
		even
Nem_MzWalls:	binclude	"artnem/MZ Walls.bin"
		even
...
Nem_SlzCannon:	binclude	"artnem/SLZ Cannon.bin"
		even
Nem_TrainBoss:	binclude	"artnem/Train Boss.bin"
		even
; ---------------------------------------------------------------------------
; Compressed graphics - SYZ stuff
; ---------------------------------------------------------------------------
Nem_Boosters:	binclude	"artnem/Boosters.bin"
		even
...
Nem_TitleCard_End:
		even
Nem_TitleCardOld:
		binclude	"artunc/Title Cards Continue.bin"
Nem_TitleCardOld_End:
		even
Nem_TitleCardResults:
		binclude	"artunc/Title Cards Results.bin"
Nem_TitleCardResults_End:
		even
Nem_IntroText:	binclude	"artnem/8x16 - Font.bin"
		even
Nem_Hud:		binclude	"artnem/HUD.bin"	; HUD (rings, time, score)
		even
Nem_Hud_Old:	binclude	"artnem/HUD - Old.bin"	; Old HUD, used in the special stage's results screen
		even
Nem_Lives:		binclude	"artnem/HUD - Life Counter Icon.bin"
		even
Nem_LivesAlt:	binclude	"artnem/HUD - Life Counter Icon NEW.bin"
		even
...
Art_Signpost:	binclude	"artunc/Signpost.bin"	; end of level signpost
		even
Art_SignpostAmy: binclude	"artunc/Signpost Amy.bin"	; end of level signpost of Amy
		even
...
Nem_ContSonic:	binclude	"artnem/Continue Screen Sonic.bin"
		even
Nem_ContAmy:	binclude	"artnem/Continue Screen Amy.bin"
		even
...
; ---------------------------------------------------------------------------
Blk16_TS:	binclude	"map16_u/TS.bin"
		even
Nem_Title:	binclude	"artnem/8x8 - Title.bin"	; Title patterns
		even
Blk256_TS:	binclude	"map256_u/TS.bin"
		even
Blk16_GHZ:	binclude	"map16_u/GHZ.bin"
		even
Kos_GHZ:	binclude	"artcom/8x8 - GHZ.bin"	; GHZ primary patterns
		even
Blk256_GHZ:	binclude	"map256_u/GHZ.bin"
		even
Blk16_LZ:	binclude	"map16_u/LZ.bin"
		even
Kos_LZ:		binclude	"artcom/8x8 - LZ.bin"	; LZ primary patterns
		even
...
Kos_MZ:		binclude	"artcom/8x8 - MZ.bin"	; MZ primary patterns
		even
Blk256_MZ:	binclude	"map256_u/MZ (JP1).bin"
		even
Blk16_SLZ:	binclude	"map16_u/SLZ.bin"
		even
Kos_SLZ:	binclude	"artcom/8x8 - SLZ.bin"	; SLZ primary patterns
		even
Blk256_SLZ:	binclude	"map256_u/SLZ.bin"
		even
Blk16_SYZ:	binclude	"map16_u/SYZ.bin"
		even
Kos_SYZ:	binclude	"artcom/8x8 - SYZ.bin"	; SYZ primary patterns
		even
Blk256_SYZ:	binclude	"map256_u/SYZ.bin"
		even
...
Blk16_BZ:	binclude	"map16_u/Tutorial.bin"
		even
Kos_BZ:		binclude	"artcom/8x8 - Tutorial.bin"
		even
Blk256_BZ:	binclude	"map256_u/Tutorial.bin"
		even
Blk16_End:	binclude	"map16_u/Ending.bin"
		even
Kos_Ending:	binclude	"artcom/8x8 - Ending.bin"	; Ending primary patterns
		even
Blk256_End:	binclude	"map256_u/Ending.bin"
		even
Blk16_UBZ:	binclude	"map16_u/UBZ.bin"
		even
Kos_UBZ:	binclude	"artcom/8x8 - UBZ.bin"	; UBZ primary patterns
		even
Blk256_UBZ:	binclude	"map256_u/UBZ.bin"
		even
Blk16_TAZ:	binclude	"map16_u/TAZ.bin"
		even
Kos_TAZ:	binclude	"artcom/8x8 - TAZ.bin"	; TAZ primary patterns
		even
Blk256_TAZ:	binclude	"map256_u/TAZ.bin"
		even
...
Nem_Eggman:	binclude	"artnem/Boss - Main.bin"
		even
Nem_EggmanBg:	binclude	"artnem/Unused - Eggman Ending.bin"
		even
Nem_AltEggman:	binclude	"artnem/Boss - Alt Main.bin"
		even
...
Nem_Sbz2Eggman:	binclude	"artnem/Boss - Eggman in SBZ2 & FZ.bin"
		even
Nem_Sbz2EggmanAlt:	binclude	"artnem/Boss - Amy's Eggman in SBZ2 & FZ.bin"
		even
...
Nem_EndSonic:	binclude	"artnem/Ending - Sonic.bin"
		even
Nem_EndAmy:	binclude	"artnem/Ending - Amy Rose.bin"
		even
...
Col_SLZ:	binclude	"collide/SLZ.bin"	; SLZ index
		even
Col_TAZ:	binclude	"collide/TAZ.bin"	; TAZ index
		even
...
Col_BS:		binclude	"collide/Tutorial.bin" ; Tutorial index
		even
Col_Ending:	binclude	"collide/Ending.bin"	; Ending index
		even
; ---------------------------------------------------------------------------
...
TitleScreen_Index:
		; Title Screen
		dc.w Level_Null-TitleScreen_Index, Level_TSbg-TitleScreen_Index, Level_Null-TitleScreen_Index
Level_Index:
		; GHZ
		...
		dc.w Level_Tutorial-Level_Index, Level_TutorialBG-Level_Index, Level_Null-Level_Index
		; LZ
		...
		dc.w Level_MZ3-Level_Index, Level_MZ3bg-Level_Index, Level_Null-Level_Index
		dc.w Level_Null-Level_Index, Level_Null-Level_Index, Level_Null-Level_Index
		; SLZ
		...
		dc.w Level_TAZ-Level_Index, Level_TAZbg-Level_Index, Level_Null-Level_Index
		; SYZ
		...
		dc.w Level_SBZ2-Level_Index, Level_SBZ2bg-Level_Index, Level_SBZ2bg-Level_Index
		dc.w Level_Null-Level_Index, Level_UBZbg-Level_Index, Level_UBZbg-Level_Index
		zonewarning Level_Index,24
		; Ending
		dc.w Level_End-Level_Index, Level_EndBG-Level_Index, Level_Null-Level_Index
		dc.w Level_End-Level_Index, Level_EndBG-Level_Index, Level_Null-Level_Index
		dc.w Level_Null-Level_Index, Level_Null-Level_Index, Level_Null-Level_Index
		dc.w Level_Null-Level_Index, Level_Null-Level_Index, Level_Null-Level_Index

Level_TSbg: binclude "levels/tsbg.bin"
		even
...
Level_GHZbg:	binclude	"levels/ghzbg.bin"
		even
Level_Tutorial:	binclude	"levels/tutorial.bin"
		even
Level_TutorialBG:	binclude	"levels/tutorialbg.bin"
		even
...
Level_SLZ3:	binclude	"levels/slz3.bin"
		even
Level_TAZ:	binclude	"levels/taz.bin"
		even
Level_TAZbg:	binclude	"levels/tazbg.bin"
		even
...
Level_End:	binclude	"levels/ending.bin"
		even
Level_EndBG:	binclude	"levels/endingbg.bin"
		even
Level_UBZbg:	binclude	"levels/ubzbg.bin"
		even
Level_Null:
		dc.l 0
...
ObjPos_AmyIndex:
		; GHZ
		dc.w ObjPos_GHZ1A-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_GHZ2A-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_GHZ3A-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_Bonus-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		; LZ
		dc.w ObjPos_LZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_LZ2-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_LZ3-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SBZ3-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		; MZ
		dc.w ObjPos_MZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_MZ2-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_MZ3-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_MZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		; SLZ
		dc.w ObjPos_SLZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SLZ2-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SLZ3-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_TAZ-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		; SYZ
		dc.w ObjPos_SYZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SYZ2-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SYZ3-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SYZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		; SBZ
		dc.w ObjPos_SBZ1-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_SBZ2-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_FZ-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_UBZ-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		zonewarning ObjPos_AmyIndex,$10
		; Ending
		dc.w ObjPos_End-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_End-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_End-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
		dc.w ObjPos_End-ObjPos_AmyIndex, ObjPos_Null-ObjPos_AmyIndex
...
ObjPos_Index:
		...
		dc.w ObjPos_Bonus-ObjPos_Index, ObjPos_Null-ObjPos_Index
		; LZ
		...
		dc.w ObjPos_TAZ-ObjPos_Index, ObjPos_Null-ObjPos_Index
		; SYZ
		...
		dc.w ObjPos_UBZ-ObjPos_Index, ObjPos_Null-ObjPos_Index
...
ObjPos_GHZ1:	binclude	"objpos/ghz1.bin"
		even
ObjPos_GHZ1A:	binclude	"objpos/ghz1A.bin"
		even
ObjPos_GHZ2:	binclude	"objpos/ghz2.bin"
		even
ObjPos_GHZ2A:	binclude	"objpos/ghz2a.bin"
		even
ObjPos_GHZ3:	binclude	"objpos/ghz3 (JP1).bin"
		even
ObjPos_GHZ3A:	binclude	"objpos/ghz3A.bin"
		even
...
ObjPos_SLZ3:	binclude	"objpos/slz3.bin"
		even
ObjPos_TAZ:	binclude	"objpos/taz.bin"
		even
...
ObjPos_End:	binclude	"objpos/ending.bin"
		even
ObjPos_Bonus:	binclude	"objpos/tutorial.bin"
		even
ObjPos_UBZ:	binclude	"objpos/ubz.bin"
		even
ObjPos_Null:	dc.b $FF, $FF, 0, 0, 0,	0
		dc.b $FF
		even

Nem_LevSel:	binclude	"artnem/Level Select BG.bin"
			even
Eni_LevSel:	binclude	"tilemaps/Level Select BG.bin"
			even
